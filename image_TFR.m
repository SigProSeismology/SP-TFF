...
subplot 321; imagesc(tt,fff, temp(1:floor(N/2)+1,:)),colormap(col), colorbar ,grid on;  title('(a)', 'FontSize', 16); caxis([0,0.65*cax]);
%set(gca,'xticklabel',{[]});
...
temp=abs(reshape(TFR2,N,N)); cax=max(max(temp));
subplot 323; imagesc(tt,fff, temp(1:floor(N/2)+1,:)),colormap(col), colorbar ,grid on;  title('(c)', 'FontSize', 16); caxis([0,0.65*cax]);
%set(gca,'xticklabel',{[]});
%ylabel('Frequency(Hz)', 'FontSize', 16);
set(gca,'FontSize',16); axis tight; axis xy
temp=abs(reshape(TFR2,N,N)); cax=max(max(temp));
...
subplot 325; imagesc(tt,fff, temp(1:floor(N/2)+1,:)),colormap(col), colorbar ,grid on;  title('(e)', 'FontSize', 16); caxis([0,0.65*cax]);
