function F_A = Amplitude(majnorm, alpha,beta,ind)
% Amplitude: A code to apply amplitude filtering 
% F_A = Amplitude(majnorm, alpha,beta,ind)
%
% 
...
A= zeros(size(majnorm,1),size(majnorm,2));  F_A=0*A;
for i = 1 : size(A,1)
    for j = 1 : size(A,2)
        A(i,j)=abs(majnorm(i,j));%(SEMI(11,i,j)+SEMI(12,i,j))/SEMI(10,i,j);
    end
end
A=A/max(max(A));
for i = 1 : size(A,1)
